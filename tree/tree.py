...
class Node(object):
    ...
    def __init__(self, X, y, depth=0, attributes=None, max_depth=None):
        X, y = Check_X_y(X, y)
        self.X = X  # 每个节点对应的训练集
        self.y = y  # 训练集对应的样本分类
...
class NodeByID3(Node):
    ...
    def __init__(self, X, y, depth=0, attributes=None, max_depth=None):
        super(NodeByID3, self).__init__(X, y, depth=depth, attributes=attributes, max_depth=max_depth)
    ...
    def _feature_choose(self, features_k):
        ...
        for attr in np.unique(self.X[:, features_k]):
            ...
            yield attr, NodeByID3(X, y, self.depth + 1, self.features)
...
class NodeByC4_dot_5(Node):
    ...
    """
    def __init__(self, X, y, depth=0, attributes=None, max_depth=None):
        super(NodeByC4_dot_5, self).__init__(X, y, depth=depth, attributes=attributes, max_depth=max_depth)
    ...
    def _feature_choose(self, features_k, value_v):
        ...
        for i in range(2):
            ...
            if not i:
                ...
                y = self.y[~index]
                attr = "{} > {}".format(self.attributes[features_k], self.features[features_k][value_v])
            yield attr, NodeByC4_dot_5(X, y, self.depth + 1, self.attributes)
    ...
    def breadth_first_fit(self):
        ...
        while depth < self.max_depth and not BFS.empty():
            ...
            if currNode.classifier is None:
                Gain_ratio = list()
                Gain_ratio_index = list()
                for i, feature in enumerate(currNode.features):
                    for j, value in enumerate(feature):
                        if currNode.feature_chosen[i][j]:
                            continue
                        Gain_ratio.append(currNode.gain_ratio(i, j))
                        Gain_ratio_index.append((i, j))
                features_k, value_v = Gain_ratio_index[Gain_ratio.index(max(Gain_ratio))]
                currNode.set_split_features(features_k, value_v)
                for child in currNode.children:
                    BFS.put(currNode.children[child])
...
class DecisionTreeClassifier(object):
    ...
    def fit(self, X, y):
        if self.criterion == 'ID3':
            self.tree_ = NodeByID3(X, y, attributes=self.attributes, max_depth=self.max_depth)
        elif self.criterion == 'C4.5':
            self.tree_ = NodeByC4_dot_5(X, y, attributes=self.attributes, max_depth=self.max_depth)
        self.tree_.fit()
        ...
        for i, e in enumerate(X):
            ...
            while currNode.classifier is None and currNode.depth < max_depth:  # 节点不是叶节点
                ...
                if e[features_k] <= currNode.features[features_k][value_v]:
                    ...
                    symbol = ">"
                attr = "{} {} {}".format(currNode.attributes[features_k],
                                         symbol, currNode.features[features_k][value_v])
                # attr = X[currNode.attributes.index(currNode.split_features)]
                currNode = currNode.children[attr]
                # if currNode.children[attr]:
                #     currNode = currNode.children[attr]
                # else:
